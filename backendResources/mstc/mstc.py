...
rootAddress = 'F:/Project/2022SRE/Consistent/backend/Consistent/mstc/'
def parse_args():
    ...
    def enumerate():
        ...
        for result in enumerator.enumerate():
            if pipe:
                ...
                data = data_args(args)
                res_data = print_result(result, args, stats, csolver.n, data)
                if res_data != []:
                    res.append(res_data)
                    # print(res)
                    if remaining:
...
def data_args(args):
    data = []
    with open(args.infile.name, 'r',encoding='utf-8') as f:
            ...
            for receiver in ready:
                while receiver.poll():
                    ...
                    if result[0] == 'done':
                        ...
                        #results.add(res_set)
                        data = []                                                                                                                                                                                                                                                                                                                                                           
                        print_result(result, args, stats, csolver.n, data)
...
def print_result(result, args, stats, num_constraints, data):
    res_data= []
    ...
    if args.verbose:
        if output == 'U' :
            for x in result[1]:
                file = rootAddress + 'encodeMapping.js'
                ctx = execjs.compile(open(file).read())
                params = ctx.call('parse',data[x-1])
                if params:
                    res_data.append(params)
    return res_data
            # output1 = "%s\n" % (" ".join([data[x] for x in result[1]]))
            # print(output1)
